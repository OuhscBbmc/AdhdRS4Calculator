...
test_that("missing_one_column", {
  ...
  for( item_number in item_numbers ) {
    ...
    # expected_error <- sprintf("assertion failed: The 'item_%02d' variable must be of class 'numeric' or 'integer'.",
    #                           item_number)
    # expected_error <- 'Error : class(item_18) %in% c("numeric", "integer") is not TRUE\n'
    expected_error <- "Error in AdhdRS4Calculator::calculate_subject.+?  : \n  The 'item_01' variable must be of class 'numeric' or 'integer'.\n"
